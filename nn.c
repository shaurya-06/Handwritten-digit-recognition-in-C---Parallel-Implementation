...
void trainNetwork(Network* network){
    ...
    int i;
    for(i=0; i<imageFileHeader.maxImages; i++){
        Image img;
...
static void feedForwardLayer(Layer* previousLayer, Layer* layer){
    ...
    // {
        int hn;
        for(hn=0; hn<layer->numberOfNodes; ++hn){
            Node* node = &layer->nodes[hn];
            node->output = node->bias;
            float temp=node->bias;
            int w;
            #pragma omp parallel for reduction(+: temp)
            for(w=0; w<previousLayer->numberOfNodes; ++w){
...
static void updateNode(Layer* previousLayer, double backPropValue, Node* node){
	int hn;
    #pragma omp parallel for 
    for(hn=0; hn<previousLayer->numberOfNodes; ++hn){
...
static void backPropagate(Network* network, int label){
    ...
    int on;
    #pragma omp parallel for
    for(on=0; on<outputLayer->numberOfNodes; ++on){
    ...
    int hn;
    #pragma omp parallel for
    for(hn=0; hn<hiddenLayer->numberOfNodes; ++hn){
...
static uint8_t getClassification(Layer* layer){
    ...
    int maxIndex = 0;
	int on;
    for(on=0; on<layer->numberOfNodes; ++on){
